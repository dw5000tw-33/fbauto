...
`;root.append(style);
  ...
  box.innerHTML='<label>① 先決定預定時間</label><button class="time-display" type="button"></button><div class="time-quick"><button type="button" data-min="30">30 分鐘後</button><button type="button" data-min="60">1 小時後</button><button type="button" data-min="120">2 小時後</button></div><div class="time-quick" style="grid-template-columns:1fr;margin-top:6px"><button type="button" data-test="2">Threads 測試：2 分鐘後</button></div><div class="time-picker" hidden><div class="calendar-head"><button type="button" data-nav="-1">‹</button><strong></strong><button type="button" data-nav="1">›</button></div><div class="calendar-grid"></div><div class="clock-row"><select class="ampm"><option value="AM">上午</option><option value="PM">下午</option></select><select class="hour"></select><select class="minute"></select></div><div class="time-hint">正式預排至少為現在 30 分鐘後；2 分鐘選項暫留供 Threads 測試。排程期間請保持電腦、瀏覽器、目標分頁及原生發文視窗開啟。</div></div>';
  ...
  display.onclick=()=>picker.hidden=!picker.hidden;box.querySelectorAll('[data-nav]').forEach(b=>b.onclick=()=>{viewMonth=new Date(viewMonth.getFullYear(),viewMonth.getMonth()+Number(b.dataset.nav),1);draw()});box.querySelectorAll('[data-min]').forEach(b=>b.onclick=()=>{selected=round5(Date.now()+Number(b.dataset.min)*60000);viewMonth=new Date(selected.getFullYear(),selected.getMonth(),1);sync()});box.querySelector('[data-test]').onclick=()=>{selected=new Date(Date.now()+2*60000);selected.setSeconds(0,0);native.value=format(selected);display.textContent='Threads 測試｜'+selected.toLocaleString('zh-TW',{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',hour12:true});setControls();viewMonth=new Date(selected.getFullYear(),selected.getMonth(),1);draw()};ampm.onchange=hour.onchange=minute.onchange=applyClock;sync();
})();
