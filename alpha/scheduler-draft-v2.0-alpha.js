...
#fb-ad-alpha-local-scheduler .draft-media img{width:78px;height:78px;object-fit:cover;border:1px solid #e24b5e;border-radius:7px;background:#08080a}
...
`;
  ...
  holder.innerHTML='<label for="s-image">預選照片（第一版限一張）</label><input id="s-image" type="file" accept="image/*"><div class="draft-media" hidden><img alt="預選照片預覽"><div class="meta"></div></div><div class="verify-note">圖片只暫存在目前分頁的記憶體；關閉或重新整理後需要重新選取。</div>';
  preview.before(holder);preview.hidden=true;
  const image=holder.querySelector('#s-image'),media=holder.querySelector('.draft-media'),thumb=holder.querySelector('img'),meta=holder.querySelector('.meta');
  ...
  const render=form=>{summary.hidden=false;summary.querySelector('.text').textContent=form.text;summary.querySelector('.when').textContent='預定時間：'+new Date(form.timeMs).toLocaleString('zh-TW')+'｜照片：'+form.file.name};
  const validate=form=>{if(!form.text){say('請先輸入發文文字。','bad');draft.focus();return false}if(!form.file){say('請先由面板選取一張照片。','bad');image.focus();return false}if(!Number.isFinite(form.timeMs)||form.timeMs<=Date.now()){say('請選擇晚於現在的預定時間。','bad');time.focus();return false}return true};
  image.onchange=()=>{clearUrl();file=image.files?.[0]||null;if(!file){media.hidden=true;return}if(!file.type.startsWith('image/')){file=null;image.value='';media.hidden=true;say('選取的檔案不是圖片。','bad');return}objectUrl=URL.createObjectURL(file);thumb.src=objectUrl;meta.textContent=file.name+'｜'+formatBytes(file.size)+'｜'+(file.type||'圖片');media.hidden=false;say('照片已由使用者選取並暫存在目前分頁。','ok')};
  check.textContent='建立草稿預覽';start.textContent='建立單筆預排';send.textContent='下一階段：寫入平台';send.disabled=true;
  check.onclick=()=>{const form=readForm();if(!validate(form))return;render(form);say('文字、照片與時間已組成一則完整預覽。','ok')};
  const stopTimer=()=>{if(timer)clearTimeout(timer);timer=null;scheduledAt=0;cancel.disabled=true;start.disabled=false};
  const tick=()=>{const remain=scheduledAt-Date.now();if(remain<=0){timer=null;cancel.disabled=true;start.disabled=false;say('預排時間已到；第一階段完成。下一階段將測試寫入平台發文視窗。','ok');return}const sec=Math.ceil(remain/1000),h=Math.floor(sec/3600),m=Math.floor(sec%3600/60),s=sec%60;say('單筆草稿已建立，倒數 '+String(h).padStart(2,'0')+':'+String(m).padStart(2,'0')+':'+String(s).padStart(2,'0')+'。目前不會送出。','ok');timer=setTimeout(tick,Math.min(1000,remain))};
  start.onclick=()=>{const form=readForm();if(!validate(form))return;stopTimer();snapshot={schema:'33-single-post-draft-v1',platform:root.querySelector('#s-platform')?.value||'',text:form.text,scheduledAt:new Date(form.timeMs).toISOString(),image:{name:form.file.name,type:form.file.type,size:form.file.size},file:form.file};render(form);scheduledAt=form.timeMs;cancel.disabled=false;start.disabled=true;tick()};
  cancel.onclick=()=>{stopTimer();snapshot=null;summary.hidden=true;say('已取消單筆預排；文字與照片仍保留在面板中。')};
