...
`;
  ...
  const topicHints={'':'請明確選擇是否加入 Threads 社群／主題。',manual:'輸入主題名稱；建立預排後依提示親自點一下原生主題欄，後續由程式完成。',none:'不加入社群／主題，交由 Threads 一般推薦系統分發（最快）。'};
  ...
  start.onclick=()=>{const form=readForm();if(!validate(form))return;const isThreads=root.querySelector('#s-platform')?.value==='threads';if(isThreads&&!topicMode.value){say('請先選擇 Threads 主題要「手動輸入」或「保留空白」。','bad');topicMode.focus();return}if(isThreads&&topicMode.value==='manual'&&!topicManual.value.trim()){say('請輸入 Threads 主題名稱。','bad');topicManual.focus();return}stopTimer();snapshot={schema:'33-single-post-draft-v2',platform:root.querySelector('#s-platform')?.value||'',text:form.text,scheduledAt:new Date(form.timeMs).toISOString(),media:{name:form.file.name,type:form.file.type,size:form.file.size,kind:form.file.type.startsWith('video/')?'video':'image'},topic:{mode:topicMode.value,value:topicManual.value.trim(),aiLabel:aiLabel.checked},file:form.file};root.__singleDraft=snapshot;scheduledAt=form.timeMs;cancel.disabled=false;start.disabled=true;lockForm(true);root.dispatchEvent(new CustomEvent('33:draft-ready'));tick()};
