...
let verified=false;
const marketplaceCard=$('#fb-ad-alpha-marketplace-card'),marketplacePanel=$('#fb-ad-alpha-marketplace'),marketplaceDate=$('#fb-ad-alpha-marketplace-date'),marketplaceToday=$('#fb-ad-alpha-marketplace-today'),marketplaceScan=$('#fb-ad-alpha-marketplace-scan'),marketplaceAuto=$('#fb-ad-alpha-marketplace-auto'),marketplaceStop=$('#fb-ad-alpha-marketplace-stop'),marketplaceMark=$('#fb-ad-alpha-marketplace-mark'),marketplaceStatus=$('#fb-ad-alpha-marketplace-status'),marketplaceResults=$('#fb-ad-alpha-marketplace-results');let marketplaceItems=[],marketplaceAutoRunning=false;
...
const scanMarketplace=()=>{
  ...
  if(!marketplaceItems.length){setMarketplaceStatus('沒有找到商品卡片。請確認目前位於 Marketplace「你的商品」頁面。','bad');return}
  setMarketplaceStatus('找到 '+marketplaceItems.length+' 個自己的商品。請設定截止日期後再標記候選。','ok');
};
...
marketplaceToday.onclick=()=>{marketplaceDate.value=new Date().toISOString().slice(0,10);marketplaceDate.focus()};
marketplaceAuto.onclick=async()=>{
  marketplaceAutoRunning=true;marketplaceAuto.disabled=true;marketplaceStop.hidden=false;marketplaceItems=[];let idle=0;
  for(let round=1;marketplaceAutoRunning&&round<=250;round++){
    const added=mergeMarketplaceItems(discoverMarketplaceItems());renderMarketplace();
    setMarketplaceStatus('自動掃描第 '+round+' 輪：已收集 '+marketplaceItems.length+' 個商品，本輪新增 '+added+' 個。','');
  ...
  marketplaceAutoRunning=false;marketplaceAuto.disabled=false;marketplaceStop.hidden=true;renderMarketplace();
  setMarketplaceStatus('自動掃描完成：共 '+marketplaceItems.length+' 個商品。若商品很多，請確認 Facebook 已改為由舊到新排序。','ok');
};
...
marketplaceMark.onclick=()=>{
  ...
  const cutoff=new Date(marketplaceDate.value+'T23:59:59'),unknown=marketplaceItems.filter(item=>!item.date).length;
  marketplaceItems.forEach(item=>item.eligible=Boolean(item.date&&item.date<=cutoff));renderMarketplace();
  const count=marketplaceItems.filter(item=>item.eligible).length;
  setMarketplaceStatus('已標記 '+count+' 個到期候選'+(unknown?'；'+unknown+' 個無法辨識刊登日期。':'')+'。尚未刪除。','ok');
};
