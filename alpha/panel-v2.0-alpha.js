...
let verified=false;
const marketplaceCard=$('#fb-ad-alpha-marketplace-card'),marketplacePanel=$('#fb-ad-alpha-marketplace'),marketplaceScan=$('#fb-ad-alpha-marketplace-scan'),marketplaceStatus=$('#fb-ad-alpha-marketplace-status'),marketplaceResults=$('#fb-ad-alpha-marketplace-results');
...
const scanMarketplace=()=>{
  ...
  let index=0;for(const item of cards.values()){
    index++;const row=document.createElement('div');row.className='result';
    const title=document.createElement('div');title.textContent=index+'. '+item.text.slice(0,190)+(item.text.length>190?'…':'');
  ...
  setMarketplaceStatus('找到 '+cards.size+' 個自己的商品。已完成只讀掃描，尚未刪除。','ok');
};
