...
const renderGroups=()=>{
  ...
  groupList.forEach((group,index)=>{
    ...
    const check=document.createElement('input');check.type='checkbox';check.checked=Boolean(group.enabled);check.onchange=()=>{group.enabled=check.checked;setGroupStatus('已選 '+groupList.filter(x=>x.enabled).length+' 個社團。','ok')};
    const name=document.createElement('span');name.textContent=group.name||group.url;name.title=group.url;
    const remove=document.createElement('button');remove.textContent='移除';remove.onclick=()=>{groupList.splice(index,1);renderGroups();setGroupStatus('已移除社團。','')};
    row.append(check,name,remove);groupListEl.appendChild(row);
  });
  setGroupStatus('共 '+groupList.length+' 個社團；已選 '+groupList.filter(x=>x.enabled).length+' 個。','ok');
};
...
groupScan.onclick=()=>{
  ...
  const oldByUrl=new Map(groupList.map(group=>[group.url,group]));for(const group of found.values())if(!oldByUrl.has(group.url))groupList.push(group);
  renderGroups();setGroupStatus(found.size?'掃描到 '+found.size+' 個可見社團；請勾選本次要使用的社團。':'沒有掃描到社團。請先前往 Facebook「你的社團」頁面再試。',found.size?'ok':'bad');
};
groupLoad.onclick=()=>groupFile.click();
groupFile.onchange=async()=>{
  const file=groupFile.files?.[0];if(!file)return;
  try{const data=JSON.parse(await file.text());if(!Array.isArray(data.groups))throw new Error();groupList=data.groups.filter(g=>g&&typeof g.url==='string').map(g=>({name:String(g.name||g.url),url:g.url,enabled:Boolean(g.enabled)}));renderGroups();setGroupStatus('已載入 '+groupList.length+' 個本機社團。','ok')}catch{setGroupStatus('JSON 格式不正確，無法載入。','bad')}finally{groupFile.value=''}
};
groupSave.onclick=()=>{if(!groupList.length){setGroupStatus('尚無社團可下載。','bad');return}downloadGroups();setGroupStatus('已下載本機 JSON 檔。','ok')};
