...
let verified=false;
const marketplaceCard=$('#fb-ad-alpha-marketplace-card'),marketplacePanel=$('#fb-ad-alpha-marketplace'),marketplaceDate=$('#fb-ad-alpha-marketplace-date'),marketplaceScan=$('#fb-ad-alpha-marketplace-scan'),marketplaceMark=$('#fb-ad-alpha-marketplace-mark'),marketplaceStatus=$('#fb-ad-alpha-marketplace-status'),marketplaceResults=$('#fb-ad-alpha-marketplace-results');let marketplaceItems=[];
...
const scanMarketplace=()=>{
  ...
  marketplaceItems=[...cards.values()].map(({card,text})=>{
    ...
    return {card,text,title,date,dateText,eligible:false};
  });
  if(!marketplaceItems.length){marketplaceResults.innerHTML='';setMarketplaceStatus('沒有找到商品卡片。請確認目前位於 Marketplace「你的商品」頁面。','bad');return}
  renderMarketplace();setMarketplaceStatus('找到 '+marketplaceItems.length+' 個自己的商品。請設定截止日期後再標記候選。','ok');
};
