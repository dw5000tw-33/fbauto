...
let verified=false;
const trackCard=$('#fb-ad-alpha-track-card'),tracker=$('#fb-ad-alpha-tracker'),keyword=$('#fb-ad-alpha-keyword'),message=$('#fb-ad-alpha-message'),scanBtn=$('#fb-ad-alpha-scan'),diagnoseBtn=$('#fb-ad-alpha-diagnose'),trackStatus=$('#fb-ad-alpha-track-status'),results=$('#fb-ad-alpha-results');
...
scanBtn.onclick=()=>{
 ...
 for(const el of source){
   ...
   if(!text.includes(word)||seen.has(text))continue;
   seen.add(text);matches.push({el,text});
 ...
 if(!matches.length){
   ...
   while((node=walker.nextNode())&&matches.length<30){
     ...
     if(!raw.includes(word)||!holder||root.contains(holder)||!visible(holder))continue;
     const el=closestPost(holder);
     const text=(el.innerText||raw).replace(/\s+/g,' ').trim()||raw;
     const key=text.length>180?text.slice(0,180):text;
     if(seen.has(key))continue;
     seen.add(key);matches.push({el,text});
   }
 }
 results.innerHTML='';
 if(!matches.length){setTrack('目前可見內容沒有找到「'+word+'」。','bad');return}
 setTrack('找到 '+matches.length+' 則候選內容。請逐篇定位並自行決定是否留言。','ok');
