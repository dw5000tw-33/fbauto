...
let verified=false;
const trackCard=$('#fb-ad-alpha-track-card'),tracker=$('#fb-ad-alpha-tracker'),keyword=$('#fb-ad-alpha-keyword'),message=$('#fb-ad-alpha-message'),rounds=$('#fb-ad-alpha-rounds'),scanBtn=$('#fb-ad-alpha-scan'),stopBtn=$('#fb-ad-alpha-stop'),diagnoseBtn=$('#fb-ad-alpha-diagnose'),trackStatus=$('#fb-ad-alpha-track-status'),results=$('#fb-ad-alpha-results');
...
const findMatches=word=>{
 ...
 const add=(el,text)=>{const clean=String(text||'').replace(/\s+/g,' ').trim(),key=clean.slice(0,220);if(!clean.includes(word)||seen.has(key))return;seen.add(key);found.push({el,text:clean})};
 const containers=[...document.querySelectorAll('article,[role="article"],[data-pagelet^="FeedUnit_"]')].filter(el=>!root.contains(el)&&visible(el)&&inFeed(el));
 containers.forEach(el=>add(el,el.innerText));
 const walker=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);let node;
 while((node=walker.nextNode())&&found.length<50){
   const raw=String(node.nodeValue||'').replace(/\s+/g,' ').trim(),holder=node.parentElement;
   if(!raw.includes(word)||!holder||root.contains(holder)||!visible(holder)||!inFeed(holder))continue;
   const el=closestPost(holder);add(el,el.innerText||raw);
 }
 return found;
};
const renderMatches=(matches,word)=>{
 results.innerHTML='';
 if(!matches.length){setTrack('掃描完成，沒有找到「'+word+'」。','bad');return}
 setTrack('掃描完成，找到 '+matches.length+' 則候選內容。請逐篇定位並自行決定是否留言。','ok');
 matches.forEach((item,index)=>{
   const row=document.createElement('div');row.className='result';
   const preview=item.text.length>150?item.text.slice(0,150)+'…':item.text;
   const title=document.createElement('div');title.textContent=(index+1)+'. '+preview;
   const locate=document.createElement('button');locate.textContent='定位貼文';
   locate.onclick=()=>{item.el.scrollIntoView({behavior:'smooth',block:'center'});item.el.style.outline='2px solid #38bdf8';setTimeout(()=>item.el.style.outline='',2500)};
   const copy=document.createElement('button');copy.textContent='複製留言';
   copy.onclick=async()=>{const text=message.value.trim();if(!text){setTrack('請先自行填入要複製的留言內容。','bad');message.focus();return}const ok=await copyText(text);setTrack(ok?'留言已複製；請由你自行貼上並送出。':'無法自動複製，請手動複製文字。',ok?'ok':'bad')};
   row.append(title,locate,copy);results.appendChild(row);
 });
};
